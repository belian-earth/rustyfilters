...
for (w in c(3L, 7L, 15L)) {
  t_mean <- time_med(function() rf_mean(m, w, na_policy = "propagate"))
  t_sd <- time_med(function() rf_focal(m, w, "sd", na_policy = "propagate"))
  t_med <- time_med(function() rf_median(m, w, na_policy = "propagate"))
  t_lee <- time_med(function() rf_lee(m, w))
  cat(sprintf(
...
if (requireNamespace("terra", quietly = TRUE)) {
  ...
  t_ours <- time_med(function() rf_mean(m, 7L, na_policy = "propagate"))
  t_terra <- time_med(function() terra::focal(r, 7, "mean", na.rm = FALSE))
  cat(sprintf(
    "mean   | rustyfilters %.3fs | terra %.3fs | %.1fx\n",
    t_ours, t_terra, t_terra / t_ours
  ))
  t_ours <- time_med(function() rf_median(m, 7L, na_policy = "propagate"))
  t_terra <- time_med(function() terra::focal(r, 7, "median", na.rm = FALSE))
  cat(sprintf(
