...
# Align the deployment target of bundled C in the Rust build with the macOS
# version R links against, so `ld` does not warn that objects were built for a
# newer macOS -- a warning `R CMD check --as-cran` flags as significant. R
# targets the major macOS version (`.0`); the installed SDK is usually a later
# patch release, which is the source of the mismatch. Empty off macOS.
.macos_deployment <- ""
